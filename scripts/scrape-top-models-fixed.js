...
/**
 * StockX Top Models (Most Active) ページのスクレイピング（修正版）
 */
class StockXTopModelsFixedScraper {
  constructor() {
    this.browser = null;
    this.page = null;
  }
  ...
  async waitForManualVerification() {
    try {
      console.log('⏳ 手動ボット検証を待機中...');
      console.log('📋 手順:');
      console.log('1. ブラウザで「Press & Hold」ボタンを長押ししてください');
      console.log('2. 検証が完了するまで待ってください');
      console.log('3. 検証完了後、このコンソールでEnterキーを押してください');
      ...
      while (Date.now() - startTime < maxWaitTime) {
        try {
          ...
          console.log('⏳ まだ検証中... 10秒待機');
          await this.page.waitForTimeout(10000);
        } catch (e) {
          console.log('検証チェック中にエラー:', e.message);
          await this.page.waitForTimeout(5000);
        }
      }
      
      console.log('⚠️ 手動検証がタイムアウトしました');
      
    } catch (error) {
      console.log('手動検証チェックエラー:', error.message);
    }
  }
...
// メイン実行関数
async function main() {
  const scraper = new StockXTopModelsFixedScraper();
...
module.exports = { StockXTopModelsFixedScraper, main };
